...
void DisplayWords(int x, char final_words[][15]);
...
label:
    ...
    if (choice == 1)
    {
        ...
        //this loop places the all the words that the user enters into the puzzle
        for (int loop = 0;loop < x;loop++)
        {
            //empties the array in which we stored a word previously and passed it into the function
        ...
        choose_again:
            ...
            //this loop makes sure that in order to place the word, the same word that has already been placed is not being picked again
            for (int z = 0;z < k;z++)
            {
            ...
            //stores the word present in 2D array into a 1D array to be passed into the function
            k += 1;
    ...
    correct_dimensions:
        ...
        char grid[row][col];
        GenerateGrid(row, col, grid);
        //this loop places all the computer stored words(in this case 6) into the puzzle
...
// FOR DISPLAYING THE WORDS ENTERED (USER OR COMPUTER-DEFINED)
void DisplayWords(int x, char final_words[][15])
{
    for (int i = 0; i < x; i++)
    {
