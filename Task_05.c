...
int main()
{
    ...
    int r_row, r_col;
    char r_word_pass[15];
    int fits_in_row, fits_in_col;
    srand(time(0));
label:
    printf("\nPress 1 to enter the words yourself into the grid or 2 for using pre-defined words: ");
    scanf("%d", &choice);
    if (choice == 1)
    {
        ...
        printf("\nPlease enter the row size for the grid: ");
        scanf("%d", &row);
        printf("\nPlease enter the column size for the grid: ");
        scanf("%d", &col);
        char grid[row][col];
        ...
        choose_again:
            ...
            k += 1;
            for (int i = 0; i < strlen(words[r_word_position]); i++)
            {
            ...
            {
                r_row = rand() % row;
                fits_in_row = strlen(words[r_word_position]) + r_row;

            } while (fits_in_row > row);
            do
            {
                r_col = rand() % col;
                fits_in_col = strlen(words[r_word_position]) + r_col;
    ...
    correct_dimensions:
        printf("\nPlease enter the row size for the grid (minimum 12): ");
        scanf("%d", &row);
        printf("\nPlease enter the column size for the grid (minimum 12): ");
        scanf("%d", &col);
        if (row < 12 || col < 12)
        {
            printf("\nYou have entered invalid dimensions for the Grid!\nPlease re-enter! ");
...
//PLACES WORDS ON THE PUZZLE GRID
void PlaceWords(int r_row, int r_col, int direction, char r_word_pass[15], int row, int col, char grid[row][col])
{
    if (direction == 0)
    {
